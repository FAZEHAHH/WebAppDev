...
<body>
    ...
    <h2 class="section-header">Section 7: Project Requirements Risks</h2>

    <form method="POST" action="calculate.php" onsubmit="return validateProjectRequirement();">
       ...
       <?php
        ...
        $sql = "SELECT * FROM project_requirement";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                for ($i = 1; $i <= 5; $i++) {
                    echo "<label>" . $i . " <input type='radio' name='project_requirement_scale[" . $row["id"] . "]' value='" . $i . "'> </label>";
                }
                echo "</div>";
                $questionNumber++;
            }
        ...
        if (isset($_POST["business_scale"])) {
            foreach ($_POST["business_scale"] as $questionId => $scale) {
                echo "<input type='hidden' name='business_scale[" . $questionId . "]' value='" . $scale . "'>";
            }
        }

        if (isset($_POST["project_management_scale"])) {
            foreach ($_POST["project_management_scale"] as $questionId => $scale) {
                echo "<input type='hidden' name='project_management_scale[" . $questionId . "]' value='" . $scale . "'>";
            }
        }
    ...
    <script>
        function validateProjectRequirement() {
            ...
            if (!isQuestionAnswered) {
                questionDiv.style.backgroundColor = '#FFCCCC'; // Highlight unanswered question
                isAllAnswered = false;
            } else {
