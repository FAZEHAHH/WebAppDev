...
<body>
    ...
    if (isset($_SESSION['owner']) && isset($_SESSION['project_name'])) {
        ...
        // Redirect back to index.php if session variables are not set
    ...
    <h2 class="section-header">Section 4: Human Resource Risks</h2>

    <form method="POST" action="business.php" onsubmit="return validateHR();">
        <?php
        // Database connection
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                for ($i = 1; $i <= 5; $i++) {
                    echo "<label>" . $i . " <input type='radio' name='hr_scale[" . $row["id"] . "]' value='" . $i . "'> </label>";
                }
                echo "</div>";
                $questionNumber++;
            }
